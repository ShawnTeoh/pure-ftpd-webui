<?php
// Menu
$menu1_act = "Мониторинг активности";
$menu2_users = "Пользователи";
$menu3_userdel = "Удалить пользователя";
$menu4_daemon = "Управление Pure-FTPd";
$menu5_settings = "Настройки";
$menu6_webuictl = "Пользователи Pure-FTPd WebUI";
// User activity
$ua_title = "Мониторинг активности";
$ua_t_title = "Активные пользователи";
$ua_t_th1 = "Пользователь";
$ua_t_th2 = "Время/Скорость";
$ua_t_th3 = "Статус";
$ua_t_th4 = "Файл/IP";
// User management
$um_title = "Управление пользователями";
$um_userform_login = "Логин";
$um_userform_status = "Статус пользователя</br>(0 - не активен, 1 - активен)";
$um_userform_pwd = "Пароль";
$um_userform_folder = "Папка пользователя</br>(оставьте поле пустым, чтобы использовать значение по умолчанию)";
$um_userform_ullimit = "Ограничение скорости загрузки на сервер KB/s, 0 - без ограничений</br>(оставьте поле пустым, чтобы использовать значение по умолчанию)";
$um_userform_dllimit = "Ограничение скорости скачивания с севера KB/s, 0 - без ограничений</br>(оставьте поле пустым, чтобы использовать значение по умолчанию)";
$um_userform_permip = "Разрешённый IP-адрес, * - любой IP-адрес</br>(оставьте поле пустым, чтобы использовать значение по умолчанию)";
$um_userform_quotasize = "Квота на размер скачанных/загруженных данных</br>(оставьте поле пустым, чтобы использовать значение по умолчанию)";
$um_userform_quotafiles = "Квота на количество скачанных/загруженных файлов</br>(оставьте поле пустым, чтобы использовать значение по умолчанию)";
$um_add_addbutton = "Добавить";
$um_add_backbutton = "Вренуться к списку пользователей";
$um_add_presultok = "Пользователь $User успешно добавлен";
$um_add_presulterror = "ОШИБКА: " . mysqli_error($db);
$um_add_checkfields = "Пользователь не добавлен, заполнены не все поля";
$um_add_checkfieldsback = "Вренуться к списку пользователей";
$um_edit_folderok = "Папка пользователя $array[User] успешно изменена";
$um_edit_foldererror = "ОШИБКА: " . mysqli_error($db);
$um_edit_loginok = "Логин $array[User] успешно изменено на $User";
$um_edit_loginerror = "ОШИБКА: " . mysqli_error($db);
$um_edit_statusok = "Статус пользователя $array[User] успешно изменён";
$um_edit_statuserror = "ОШИБКА: " . mysqli_error($db);
$um_edit_passwd = "Пароль пользователя $array[User] успешно изменён";
$um_edit_passwderror = "ОШИБКА: " . mysqli_error($db);
$um_edit_ullimitok = "Ограничение скорости загрузки на сервер для пользователя $array[User] успешно изменёно";
$um_edit_ullimiterror = "ОШИБКА: " . mysqli_error($db);
$um_edit_dllimitok = "Ограничение скорости скачивания с сервера для пользователя $array[User] успешно изменёно";
$um_edit_dllimiterror = "ОШИБКА: " . mysqli_error($db);
$um_edit_permipok = "Разрешённый IP адрес для пользователя $array[User] успешно изменён";
$um_edit_permiperror = "ОШИБКА: " . mysqli_error($db);
$um_edit_quotasizeok = "Квота на размер скачанных/загруженных данных для пользователя $array[User] успешно изменена";
$um_edit_quotasizeerror = "ОШИБКА: " . mysqli_error($db);
$um_edit_quotafilesok = "Квота на количество скачанных/загруженных файлов для пользователя $array[User] успешно изменена";
$um_edit_quotafileserror = "ОШИБКА: " . mysqli_error($db);
$um_edit_nochanges = "Вы не вносили изменений";
$um_edit_nochangesback = "Вренуться к списку пользователей";
$um_t_title = "Выберите пользователя для редактирования";
$um_t_th1 = "Логин";
$um_t_th2 = "Статус";
$um_t_th3 = "Папка";
$um_t_th4 = "upload limit (kb/s)";
$um_t_th5 = "download limit (kb/s)";
$um_t_th6 = "Разрешённый IP";
$um_t_th7 = "QuotaSize";
$um_t_th8 = "QuotaFiles";
$um_adduserbutton = "Добавить пользователя";
$um_edit_savebutton = "Сохранить изменения";
$um_edit_backbutton = "Вренуться к списку пользователей";
// Drop user
$du_title = "Удаление пользователей";
$du_success = "Пользователь удален";
$du_error = "ОШИБКА: " . mysqli_error($db);
$du_notchecked = "Не выбран пользователь";
// Del user
$del_title = "Удаление пользователей";
$del_selecttitle = "Выберите пользователя для удаления";
$del_button = "Удалить пользователя";
// Daemon control
$dc_title = "Управление Pure-FTPd";
$dc_confeditbackbutton = "Назад";
$dc_confeditsavebutton = "Сохранить";
$dc_confeditsuccess = "Изменения сохранены, для принятия изменений перезапустите демон Pure-FTPd";
$dc_confeditnorights = "Конфиг небыл изменён, нехватает прав для записи в файл";
$dc_daemoncontrol = "Управление демоном";
$dc_wrongcommand = "Передана неверная команда демону Pure-FTPd";
$dc_wrongcommandback = "Назад";
$dc_dctitle = "Управление демоном Pure-FTPd";
$dc_dcperformbutton = "Выполнить";
$dc_dceditconfig = "Править конфиг";
// WebUI users
$wu_title = "Пользователи Pure-FTPd WebUI";
$wu_select = "Выберите пользователя для редактирования";
$wu_editbutton = "Редактировать";
$wu_adduserbutton = "Добавить пользователя";
$wu_add_resultok = "Пользователь успешно добавлен";
$wu_add_resulterror = "ОШИБКА: " . mysqli_error($db);
$wu_add_checkfields = "Пользователь не добавлен, заполнены не все поля";
$wu_add_checkfieldsback = "Вренуться к списку пользователей";
// Edit WebUI users
$ewu_form_login = "Логин";
$ewu_form_pwd = "Пароль";
$ewu_form_lang = "Язык";
$ewu_save = "Сохранить изменения";
$ewu_edit_loginok = "Логин $array[user] успешно изменено на $user";
$ewu_edit_loginerror = "ОШИБКА: " . mysqli_error($db);
$ewu_edit_passwdok = "Пароль пользователя $array[user] успешно изменён";
$ewu_edit_passwderror = "ОШИБКА: " . mysqli_error($db);
$ewu_edit_languageok = "Язык интерфейса для пользователя $array[user] успешно изменён";
$ewu_edit_languageerror = "ОШИБКА: " . mysqli_error($db);
$ewu_edit_nochanges = "Вы не вносили изменений";
$ewu_edit_nochangesback = "Вренуться к списку пользователей";
$ewu_deluserbutton = "Удалить пользователя";
$ewu_del_resultok = "Пользователь удален";
$ewu_del_resulterror = "ОШИБКА: " . mysqli_error($db);
$ewu_del_notchecked = "Не выбран пользователь";
// Settings
$settings_title = "Настройки";
$settings_ftp_dir_ok = "Значение по умолчанию для FTP директории успешно изменено";
$settings_ftp_dir_error = "ОШИБКА: " . mysqli_error($db);
$settings_upload_speed_ok = "Значение по умолчанию для ограничения скорости загрузки на сервер успешно изменено";
$settings_upload_speed_error = "ОШИБКА: " . mysqli_error($db);
$settings_download_speed_ok = "Значение по умолчанию для ограничения скорости скачивания с сервера успешно изменено";
$settings_download_speed_error = "ОШИБКА: " . mysqli_error($db);
$settings_quota_size_ok = "Значение по умолчанию для квоты на размер скачанных/загруженных данных успешно изменено";
$settings_quota_size_error = "ОШИБКА: " . mysqli_error($db);
$settings_quota_files_ok = "Значение по умолчанию для квоты на количество скачанных/загруженных файлов успешно изменено";
$settings_quota_files_error = "ОШИБКА: " . mysqli_error($db);
$settings_permitted_ip_ok = "Значение по умолчанию для разрешённого IP-адреса успешно изменено";
$settings_permitted_ip_error = "ОШИБКА: " . mysqli_error($db);
$settings_pureftpd_conf_path_ok = "Путь к конфигурационному файлу Pure-FTPd успешно изменён";
$settings_pureftpd_conf_path_error = "ОШИБКА: " . mysqli_error($db);
$settings_pureftpd_init_script_path_ok = "Путь к инит скрипту Pure-FTPd успешно изменён";
$settings_pureftpd_init_script_path_error = "ОШИБКА: " . mysqli_error($db);
$settings_pureftpwho_path_ok = "Путь к скрипту Pure-FTPWHO успешно изменён";
$settings_pureftpwho_path_error = "ОШИБКА: " . mysqli_error($db);
$settings_title2 = "Настройки Pure-FTPd WebUI";
$settings_ftp_dir = "Значение по умолчанию для FTP директории";
$settings_upload_speed = "Значение по умолчанию для ограничения скорости загрузки на сервер (KB/s)";
$settings_download_speed = "Значение по умолчанию для ограничения скорости скачивания с сервера (KB/s)";
$settings_quota_size = "Значение по умолчанию для квоты на размер скачанных/загруженных данных";
$settings_quota_files = "Значение по умолчанию для квоты на количество скачанных/загруженных файлов";
$settings_permitted_ip = "Значение по умолчанию для разрешённого IP-адреса (* - любой IP-адрес)";
$settings_pureftpd_conf_path = "Путь к конфигурационному файлу Pure-FTPd";
$settings_pureftpd_init_script_path = "Путь к инит скрипту Pure-FTPd";
$settings_pureftpwho_path = "Путь к скрипту Pure-FTPWHO (статистика)";
$settings_save_button = "Сохранить настройки";
$settings_nochanges = "Вы не вносили изменений";
?>